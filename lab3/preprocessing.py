...
def extract_log_mel_feats(
    set_type, path_to_csv, path_to_files, out_path, sr, fft_size, hop, n_mels
):
    """
    ...
    if set_type == "train":
        meta = pd.read_csv(
            path_to_csv, skiprows=1, names=["fname", "label", "manually_verified"]
        )
        ...
        for i, (file_name, label) in tqdm(enumerate(zip(file_names, labels))):
            ...
            mel_spec = melspectrogram(
                wav_data, n_fft=fft_size, hop_length=hop, n_mels=n_mels, fmax=sr // 2
            )
        ...
        for i, file_name in tqdm(enumerate(os.listdir(path_to_files))):
            ...
            mel_spec = melspectrogram(
                wav_data, n_fft=fft_size, n_mels=n_mels, fmax=sr // 2
            )
            log_mel_spec = power_to_db(mel_spec, ref=np.max)
            feats.append(
                {
